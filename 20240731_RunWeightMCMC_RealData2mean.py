...
def run_mcmc(save_dir):
    ...
    var_esc = calculate_variance_sample(esc_maps_interp, esc_median_map)
    var_cnc = calculate_variance_sample(cnc_maps_interp, cnc_median_map)

    templates_flatten = np.vstack([esc_median_map.flatten(), cnc_median_map.flatten()])
    
    measurement_error_esc = [calculate_variance_sample(esc_maps_interp, esc_median_map)**0.5,
                             calculate_variance_sample(esc_maps_interp, cnc_median_map)**0.5]
    measurement_error_cnc = [calculate_variance_sample(cnc_maps_interp, esc_median_map)**0.5,
                             calculate_variance_sample(cnc_maps_interp, cnc_median_map)**0.5]
    measurement_error_all = [calculate_variance_sample(all_maps_interp, esc_median_map)**0.5,
                             calculate_variance_sample(all_maps_interp, cnc_median_map)**0.5]
...
if __name__ == '__main__':
    save_dir = '/mnt/home/tudomlumleart/ceph/01_ChromatinEnsembleRefinement/chromatin-ensemble-refinement/MCMC_results/20240731_MCMC_2med_diff_var'
