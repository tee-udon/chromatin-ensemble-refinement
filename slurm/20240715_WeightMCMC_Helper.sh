...
SLURM_JOB_SCRIPT_TEMPLATE="/mnt/home/tudomlumleart/ceph/01_ChromatinEnsembleRefinement/chromatin-ensemble-refinement/scripts/slurm/20240715_WeightMCMC.sh"
...
do
    ...
    COMMON_PART=$(echo "$FILENAME" | cut -d'_' -f1-7 | sed 's/\.[0-9]*$//')

    # Check if the results directory already exists
    RESULTS_DIR="$PROCESSED_DIRECTORY/$COMMON_PART"
