...
def run_mcmc(save_dir, num_metastructures):
    # Load observations 
    ...
    templates_flatten = np.array([x.flatten() for x in distance_maps_bins])
...
if __name__ == '__main__':
    save_dir = '/mnt/home/tudomlumleart/ceph/01_ChromatinEnsembleRefinement/chromatin-ensemble-refinement/MCMC_results/20240815_WeightMCMC_stack_contact_diff/'
    
    num_metasturctures_list = [5, 10, 20, 50, 100]
    for num_metastructures in num_metasturctures_list:
        print(f"Running MCMC for {num_metastructures} metastructures ...")
        new_dir = os.path.join(save_dir, f'{num_metastructures}_metastructures')
        if not os.path.exists(new_dir):
            os.makedirs(new_dir)
        run_mcmc(new_dir, num_metastructures)
